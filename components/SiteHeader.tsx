...
export default function SiteHeader({ isFooterVisible = false }: SiteHeaderProps) {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${isFooterVisible ? "-translate-y-full" : "translate-y-0"} ${scrolled ? "border-b border-slate-100 bg-white/98 backdrop-blur-xl shadow-sm" : "border-b border-white/10 bg-transparent backdrop-blur-sm"}`}
    >
